...
class Menu:
    ...
    def setup(self):
        ...
        self.main_rect = pygame.Rect(self.left, self.menu_top, self.width, self.total_height)
        # buy/sell text
        self.buy_text = self.font.render('buy', False, 'Green')
    ...
    def show_entry(self, text_surf, amount, top, selected):
        ...
        self.display_surface.blit(text_surf, text_rect)
        #amount
        amount_surf = self.font.render(str(amount), False, 'Brown')
        amount_rect = amount_surf.get_rect(midright = (self.main_rect.right - 20,bg_rect.centery))
        self.display_surface.blit(amount_surf, amount_rect)
        #selected
        if selected:
            pygame.draw.rect(self.display_surface, 'Brown', bg_rect, 4, 4)
            if self.index <= self.sell_border: #sell things
